...
function preload() {
  buttonImageA = loadImage("data/borde.png", img => { img.resize(200, 100); });
  buttonImageB = loadImage("data/borde.png", img => { img.resize(200, 100); });
...
function draw() {
  ...
  if (juego.gameOver) {
    ...
    currentScreen.updateButtonText(); // Call the new function here
    currentScreen.draw();
  }
}
  ...
  if (clickedButton !== null) {
    if (juego.gameOver) {
      ...
      if (nextScreenIndex !== null) {
        ...
        if (currentScreen.question === "Pregunta 21") {
          juego.reiniciar();
          juego.setup();
          juego.iniciarMinijuego = true;
          console.log("Iniciando minijuego en Pregunta 21");
        }
      } else {
        console.log("¡Fin del juego!");
      }
